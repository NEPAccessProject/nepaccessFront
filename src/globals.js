...
const Globals = {
    ...
    doFilter: (searcherState, searchResults, preFilterCount, legacyStyle) => {
        ...
        if (searcherState.isFast41) {
            isFiltered = true;
            // this function takes an array of results with each result containing an array of records and determines if any of the records have the property  Fast41 set to true.  If so return the original result object.
            
            filteredResults = filteredResults.filter((res) => {
                const records = res.records.filter((rec => {
                    if (rec.isFast41 === true) {
                        return true;
                    }
                    else {
                        return false;
        ...
        // filteredResults = filteredResults.records.filter((result)=>{    
        //     console.log(`isFast41: ${result.isFast41}`)
        //     return result.isFast41 === true;
        // });
        console.log(`POST FILTER COUNT ${filteredResults.length}`)
...
const matches = (field, val) => {
    ...
    console.log(`matches ~ field,val:`, field, val);
    return function (a) {
        const matches = a.records.forEach(item => {
            if (item[field] === val) {
                return true
            }
            else {
