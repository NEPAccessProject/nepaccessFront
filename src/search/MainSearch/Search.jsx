...
const startNewSearch = (searchState) => {
    ...
    searchState = searchState; // for live filtering
...
//        results: [],
        ...
        axios({
            ...
            if (responseOK) {
                ...
                console.log("🚀 ~ file: Search.jsx:722 ~ startSearch ~ response.data in new state:", setSearchState.results)
            } else {
                console.log(`Unhandled response.status ${respons.status} - with response data` , response.data);
                return null;
            }
        }).then(currentResults => {
            let _data = [];
            console.log("Raw results data",data,"current Results", currentResults.length);
            if (currentResults && currentResults[0] && currentResults[0].doc) {

                _data = currentResults
                    console.log("🚀 ~ file: Search.jsx:730 ~ startSearch ~ _data:", _data)
                ...
                , 
                    ...
                    if (Globals.isEmptyOrSpaces(dataToPass.title) ||
                        (searchState.searcherInputs.searchOption && searchState.searcherInputs.searchOption === "C")) {
                        ...
                        gatherFirstPageHighlightsThenFinishSearch(_searchId, searchState.searcherInputs, searchState.results);
                    }
            } else {
...
const gatherSpecificHighlights = (_index, record) => {
    ...
    setSearchState({
      ...searchState,
        ...
        console.log("terms, last", _searchTerms, searchState.lastSearchedTerm);
...
const check = () => { // check if JWT is expired/invalid
    setSearchState({                   ...searchState,
      loaded: false, down: false });
